...
const Laboratory = (props) => {
  ...
  useEffect(() => {
    ...
    tests.dateResultReceived =
      props.activeContent.obj.dateResultReceived !== null &&
      props.activeContent.obj.dateResultReceived !== ""
        ? moment(props.activeContent.obj.dateResultReceived).format(
            "YYYY-MM-DD HH:MM:SS"
          )
        : "";
    showObj.isPcr =
      props.activeContent.obj.pcrLabName !== null &&
      props.activeContent.obj.pcrLabName !== ""
        ? true
        : false;
    showObj.hasResult =
      props.activeContent.obj.result !== "" &&
      props.activeContent.obj.dateResultReceived !== null
        ? true
        : false;
  ...
  const validate = () => {
    ...
    temp.sampleCollectedBy = tests.sampleCollectedBy
      ? ""
      : "This field is required";
    ...
    showResult &&
      (temp.dateAssayedBy = tests.dateAssayedBy
        ? ""
        : "This field is required");

    showPcrLabDetail &&
      (temp.dateCheckedBy = tests.dateCheckedBy
        ? ""
        : "This field is required");
    ...
    showPcrLabDetail &&
      (tests.sampleLoggedRemotely === "1" ||
        tests.sampleLoggedRemotely === 1) &&
      (temp.dateSampleLoggedRemotely = tests.dateSampleLoggedRemotely
        ? ""
        : "This field is required");
    showPcrLabDetail &&
      (temp.dateReceivedAtPcrLab = tests.dateReceivedAtPcrLab
        ? ""
        : "This field is required");
    //showPcrLabDetail && (temp.dateOrderBy = tests.dateOrderBy ? "" : "This field is required")
    showPcrLabDetail &&
      (temp.pcrLabSampleNumber = tests.pcrLabSampleNumber
        ? ""
        : "This field is required");
    showPcrLabDetail &&
    ...
    showPcrLabDetail &&
      (temp.sampleLoggedRemotely = tests.sampleLoggedRemotely
        ? ""
        : "This field is required");
    showPcrLabDetail &&
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      tests.dateResultReceived =
        tests.dateResultReceived !== null && tests.dateResultReceived !== ""
          ? moment(tests.dateResultReceived).format("YYYY-MM-DD HH:MM:SS")
          : "";
      tests.dateReceivedAtPcrLab =
        tests.dateReceivedAtPcrLab !== null && tests.dateReceivedAtPcrLab !== ""
          ? moment(tests.dateReceivedAtPcrLab).format("YYYY-MM-DD HH:MM:SS")
          : "";
      axios
        .put(
          `${baseUrl}laboratory/vl-results/${props.activeContent.obj.id}`,
          tests,
          { headers: { Authorization: `Bearer ${token}` } }
        )
        .then((response) => {
          ...
          setSaving(false);
          toast.success("Viral Load order & result updated successful!", {
