...
const LabHistory = (props) => {
    ...
    try {
      if (row.sampleNumber) {
        ...
        const response = await axios.get(
          `${baseUrl}lims/sample/result/${modifiedSampleNumber}`,
          {
        ...
        if (!row.result && testResult) {
          ...
          const updatedRow = {
            ...row,
            result: testResult,
            dateResultReceived: moment(resultDate).format(
              "YYYY-MM-DD HH:mm:ss"
            ),
  ...
  const LoadDeletePage = (row) => {
    setSaving(true);
    axios
      .delete(`${baseUrl}laboratory/rde-orders/tests/${row.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
    ...
    <div>
      ...
      <MaterialTable
        ...
        columns={[
          ...
          {
            title: "Date Result Received",
            field: "dateResultReceived",
            filtering: false,
          },
        ...
        data={props.orderList.map((row) => ({
          ...
          dateResultReceived:
            results[`${row.sampleNumber}_dateReceived`] ||
            row.dateResultReceived ||
            "No Date Available",
          result:
            results[`${row.sampleNumber}_result`] ||
            row.result ||
            "No Result Yet",
                  ...
                  <Button style={{ backgroundColor: "rgb(153,46,98)" }} primary>
                    <Dropdown item text="Action">
                      <Dropdown.Menu style={{ marginTop: "10px" }}>
                        <Dropdown.Item onClick={() => onClickHome(row, "view")}>
                          <Icon name="eye" />
                          View
                        </Dropdown.Item>
                        <Dropdown.Item
                          onClick={() => onClickHome(row, "update")}
                        >
                          <Icon name="edit" />
                          Update
                        </Dropdown.Item>
      ...
      >
        ...
        <Modal.Body>
          <h4>
            ...
            <b>{record && record.sampleNumber}</b>
          </h4>
        </Modal.Body>
        <Modal.Footer>
          <Button
