...
const FindPatientActions = memo(({ row }) => {
  // 1. Extract the new flags directly from the row (provided by your updated JPA query)
  ...
  // 2. Logic: If both flags are true, show the Dashboard immediately
  ...
  // 4. Otherwise, render the Enroll Button
  return (
      <>
        <ButtonGroup
            variant="contained"
            onClick={handleEnrollClick}
            style={{ backgroundColor: "rgb(153, 46, 98)", height: "30px", width: "215px", cursor: "pointer" }}
            size="large"
        >
          <Button style={{ backgroundColor: "rgb(153, 46, 98)" }}><TiArrowForward /></Button>
          <Button style={{ backgroundColor: "rgb(153, 46, 98)" }}>
          <span style={{ fontSize: "12px", color: "#fff", fontWeight: "bolder" }}>
            Enroll Patient
          </span>
