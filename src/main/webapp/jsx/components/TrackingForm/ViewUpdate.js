...
const Tracking = (props) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (
      ...
      objValues.setObjValues({ ...objValues, ["dateOfDiscontinuation"]: " " });
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      if (attemptList.length > 0) {
        ...
        setSaving(true);
        axios
          .put(`${baseUrl}patient-tracker/${props.patientObj.id}`, objValues, {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then((response) => {
            setSaving(false);
  ...
  useEffect(() => {
    console.log("patient ID: ", props.patientObj.id);
  });
  return (
    <div>
