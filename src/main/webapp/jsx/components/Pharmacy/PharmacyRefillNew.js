...
const Pharmacy = (props) => {
  ...
  useEffect(() => {
    ...
    setRegimenList(
      Object.entries(
        ...
        name: value.label,
        dispenseQuantity:
          objValues.refillPeriod !== null ? objValues.refillPeriod : "",
      }))
    );
    CheckEACStatus();
    VitalSigns();
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${baseUrl}hiv/art/pharmacy/devolve/patient/${patientObj.personUuid}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      )
      ...
      .then((response) => {
        ...
        if (matchedRecord) {
          setTptCareAndSupportRegimen(
            matchedRecord.data.tptMonitoring.tptRegimen
          );
  ...
  const handleSubmit = (e) => {
    ...
    if (observeDate) {
      ...
      toast.error(
        "No Chronic Care filled for this Encounter Date, please try again...",
        { position: toast.POSITION.BOTTOM_CENTER }
      );
      setSaving(false);
    }
