...
const EnrollmentAndCommencementUpdate = (props) => {
  ...
  const fetchExistingData = async () => {
    ...
    try {
      ...
      // Fetch regimens for the selected regimen line if available
      if (data.regimenLineId) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Render (Same as Index.js but with "Update" in title)
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <Card
      ...
      <CardContent>
        ...
        >
          <Typography sx={{ color: "#fff", fontWeight: 700, fontSize: "16px" }}>
            ART Enrollment &amp; Commencement (Update)
            {isPediatric && (
              <span
                style={{
                  marginLeft: "12px",
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            <MatButton
              ...
              disabled={saving}
            >
              <span style={{ textTransform: "capitalize" }}>{saving ? "Updating..." : "Update"}</span>
            </MatButton>
          </Box>
        </form>
      </CardContent>
