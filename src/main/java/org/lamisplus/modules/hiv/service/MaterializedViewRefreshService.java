...
public class MaterializedViewRefreshService {
    ...
    /**
     * Initialize the materialized view (create if doesn't exist)
     */
    @Transactional
    public void initializeMaterializedView() {
        try {
            log.info("Initializing materialized view mv_viral_load_eligibility");
            ...
            if (count != null && count > 0) {
                log.info("Materialized view already exists. Dropping and recreating...");
                jdbcTemplate.execute("DROP MATERIALIZED VIEW mv_viral_load_eligibility");
            }
            ...
            // Create materialized view with the full query from repository
            log.info("Creating materialized view...");
            String createViewSql = "CREATE MATERIALIZED VIEW mv_viral_load_eligibility AS " +
            ...
            log.info("Creating indexes on materialized view...");
            // Create UNIQUE index for concurrent refresh support
            jdbcTemplate.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_vl_patient_id_unique ON mv_viral_load_eligibility(patientId)");
            jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_mv_vl_facility_patient ON mv_viral_load_eligibility(facilityId, patientId)");
            jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_mv_vl_patient_uuid ON mv_viral_load_eligibility(patientUuid)");

            // Initial refresh
            log.info("Populating materialized view with initial data...");
            jdbcTemplate.execute("REFRESH MATERIALIZED VIEW mv_viral_load_eligibility");

            log.info("Materialized view initialization completed successfully");
        } catch (Exception e) {
            log.error("Error initializing materialized view", e);
            throw new RuntimeException("Failed to initialize materialized view", e);
        }
    }

    /**
     * Refresh viral load eligibility materialized view
     * Runs every day at 2:00 AM to ensure data is fresh
    ...
    public void refreshViralLoadEligibilityView() {
        try {
            ...
            log.info("Successfully refreshed mv_viral_load_eligibility materialized view in {} ms", duration);
        } catch (Exception e) {
            log.error("Error refreshing mv_viral_load_eligibility materialized view", e);
            // Don't throw exception to prevent scheduler from stopping
        }
    }

    /**
     * Manual refresh method that can be called on-demand
     * Can be triggered via API endpoint or admin interface
     */
    @Transactional
    public void manualRefreshViralLoadEligibilityView() {
        try {
            log.info("Manual refresh of mv_viral_load_eligibility materialized view triggered");
            long startTime = System.currentTimeMillis();

            jdbcTemplate.execute("REFRESH MATERIALIZED VIEW CONCURRENTLY mv_viral_load_eligibility");

            long duration = System.currentTimeMillis() - startTime;
            log.info("Manual refresh of mv_viral_load_eligibility completed in {} ms", duration);
        } catch (Exception e) {
            log.error("Error during manual refresh of mv_viral_load_eligibility materialized view", e);
    ...
    public String getLastRefreshTime() {
        try {
            return jdbcTemplate.queryForObject(
                    "SELECT last_refresh FROM pg_stat_user_tables WHERE relname = 'mv_viral_load_eligibility'",
                    String.class
            );
        } catch (Exception e) {
